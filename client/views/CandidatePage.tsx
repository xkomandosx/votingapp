...
class CandidatePageState extends CandidatePageModel {
  addName: string = "";
  storedValue: number;
}
...
export default class CandidatePage extends React.Component<
  ...
  CandidatePageState
> {
  state: CandidatePageState = new CandidatePageState();
  vm: IDotnetifyVM;
  dispatch: (state: any) => void;

  constructor(props: any) {
    super(props);
    this.vm = dotnetify.react.connect("CandidateTable", this);
  ...
  render() {
    let { addName, Candidates, Pages, SelectedPage, ShowNotification } =
      this.state;
    ...
    const handleUpdate = (candidate: {
      ...
      let newState = Candidates.map((item) =>
        item.Id === candidate.Id ? Object.assign(item, candidate) : item
      );
      this.setState({ Candidates: newState });
      this.dispatch({ Update: candidate });
    };
    ...
    return (
      <div style={styles.page}>
        <ThemeProvider theme={defaultTheme}>
          <BasePage title="Candidates" navigation="">
            <div>
              <div>
                <Fab
                  onClick={handleAdd}
              ...
              <Table>
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell style={styles.columns.votes}>Votes</TableCell>
                ...
                <TableBody>
                  {Candidates.map((item) => (
                    <TableRow key={item.Id}>
                      ...
                      <TableCell style={styles.columns.votes}>
                        {item.Votes}
                      </TableCell>
